...
import os
import json
class Classification_Task(AbstractTask):
    ...
    def dataset_load(self, **kwargs: Any):
        ...
        for file in os.listdir('data/CLS/'):
            if file.endswith('.json'):
                with open('data/CLS/' + file, 'r') as f:
                    data_list.extend([json.loads(line) for line in f.readlines()])
        self.dataset = data_list
        print('Dataset loaded')
        self.data_loaded = True
    def evaluate(self, model: LLM, args: Any, **kwargs: Any):
        ...
        if args.save_result:
            with open('result_cls_' + model_name + '_' + '.json', 'a+', encoding='utf-8') as f:
