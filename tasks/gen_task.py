...
class Generation_Task(AbstractTask):
    ...
    def dataset_load(self, **kwargs: Any):
        ...
        for file in os.listdir('data/GEN/'):
            if file.endswith('.json'):
                with open('data/GEN/' + file, 'r') as f:
    ...
    def evaluate(self, model: LLM, args: Any, task='content', **kwargs: Any):
        assert self.data_loaded
        assert task in ['content', 'qa','price']
        ...
        if task == 'content':
            # calculate bleu score and rouge score
            Bleu_score = 0.0
            Rouge_score = 0.0
            for i in range(len(results)):
